...
spec:
  ...
  - name: __CONTAINER_NAME__
    ...
    ports:
     - containerPort: __CONTAINER_PORTS__
    args: __CONTAINER_ARGS__
  affinity:
    nodeAffinity:
...
while true; do
  ...
  if [[ -n "$DRAINED_NODES" ]]; then
	...
	CONTAINER_PORTS=$(echo "$POD_DETAILS" | jq -r '.spec.containers[0].ports[0].containerPort | tojson')
	CONTAINER_ARGS=$(echo "$POD_DETAILS" | jq -r '.spec.containers[0].args | tojson')
	...
	echo "Debugging variables:"
	 ...
	 -e "s/__CONTAINER_IMAGE__/$(printf '%s' "$CONTAINER_IMAGE" | sed 's/[&/\]/\\&/g')/" \
         -e "s/__CONTAINER_PORTS__/$(printf '%s' "$CONTAINER_PORTS" | sed 's/[&/\]/\\&/g')/" \
	 -e "s/__CONTAINER_ARGS__/$(printf '%s' "$CONTAINER_ARGS" | sed 's/[&/\]/\\&/g')/" \
	...
	echo "Generated Pod spec for $POD_NAME:"
