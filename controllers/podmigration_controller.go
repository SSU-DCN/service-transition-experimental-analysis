...
func (r *PodmigrationReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	if annotations["snapshotPolicy"] == "live-migration" && annotations["sourcePod"] != "" {
		...
		log.Info("", "Live-migration", "Step 1 - Check source pod is exist or not - completed", "duration", time.Since(startTime).Milliseconds())
		...
		log.Info("", "Live-migration", "Step 2 - checkpoint source Pod - completed", "duration", time.Since(startCheckpoint).Milliseconds())
		...
		log.Info("", "Live-migration", "Step 4 - Restore destPod from sourcePod's checkpointed info - completed", "duration", time.Since(startRestore).Milliseconds())
		// time.Sleep(5)
		startWaitRunning := time.Now()
		for {
			status, _ := r.checkPodExist(ctx, newPod.Name, req.Namespace)
			if status != nil {
				log.Info("", "Live-migration", "Step 4.1 - Check whether if newPod is Running or not - completed"+status.Name+string(status.Status.Phase), "duration", time.Since(startWaitRunning).Milliseconds())
				break
			} else {
				time.Sleep(200 * time.Millisecond)
			}
		}
		log.Info("", "Live-migration", "Step 4.1 - Check whether if newPod is Running or not - completed", "duration", time.Since(startWaitRunning).Milliseconds())
		...
		startDeleteSource := time.Now()
		if err := r.deletePod(ctx, sourcePod); err != nil {
			log.Error(err, "unable to delete", "source pod", sourcePod)
			return ctrl.Result{}, err
		}
		log.Info("", "Live-migration", "Step 6 - Delete the source pod - completed", "duration", time.Since(startDeleteSource).Milliseconds())
...
func (r *PodmigrationReconciler) deletePod(ctx context.Context, pod *corev1.Pod) error {
	...
	timeout := time.After(30 * time.Second) // Set a timeout for the wait
	...
	for {
		...
		case <-ticker.C:
			...
			err := r.Get(ctx, client.ObjectKey{Name: pod.Name, Namespace: pod.Namespace}, &deletedPod)
...
func (r *PodmigrationReconciler) checkpointPod(ctx context.Context, pod *corev1.Pod, snapshotPath string) error {
	...
	r.Log.Info("Checkpoint process started", "pod", pod.Name, "snapshotPath", snapshotPath)

	// Wait for the checkpoint files to be created
	container := pod.Spec.Containers[0].Name
	checkpointDir := path.Join(snapshotPath, strings.Split(pod.Name, "-")[0], container)

	r.Log.Info("Waiting for checkpoint to complete", "checkpointDir", checkpointDir)

	checkpointWaitStart := time.Now()
	timeout := time.After(5 * time.Minute) // Set a timeout for checkpointing
	ticker := time.NewTicker(500 * time.Millisecond)
	defer ticker.Stop()
	for {
		select {
		case <-timeout:
			return fmt.Errorf("checkpoint creation timed out for pod %s", pod.Name)
		case <-ticker.C:
			if r.isCheckpointComplete(checkpointDir) {
				r.Log.Info("Checkpoint creation complete", "checkpointDir", checkpointDir, "waitTime", time.Since(checkpointWaitStart).Milliseconds())
				r.Log.Info("Total checkpoint process duration", "pod", pod.Name, "duration", time.Since(startCheckpoint).Milliseconds())
				return nil
			}
		}
	}
...
func (r *PodmigrationReconciler) isCheckpointComplete(checkpointDir string) bool {
	criticalFiles := []string{"descriptors.json", "inventory.img"} // List of critical checkpoint files
	for _, file := range criticalFiles {
		if _, err := os.Stat(path.Join(checkpointDir, file)); os.IsNotExist(err) {
			return false
		}
	}
	return true
...
func (r *PodmigrationReconciler) restorePod(ctx context.Context, pod *corev1.Pod, sourcePod, checkpointPath string) (*corev1.Pod, error) {
	...
	// Generate a new pod name
	startRestore := time.Now()
	r.Log.Info("Restore process started", "sourcePod", sourcePod, "checkpointPath", checkpointPath)
	...
	// Validate checkpoint
	r.Log.Info("Validating checkpoint before restore", "checkpointPath", checkpointPath)
	checkpointValidationStart := time.Now()
	if !r.isCheckpointComplete(checkpointPath) {
		return nil, fmt.Errorf("checkpoint validation failed, files missing in %s", checkpointPath)
	}
	r.Log.Info("Checkpoint validation completed", "checkpointPath", checkpointPath, "validationTime", time.Since(checkpointValidationStart).Milliseconds())
	// Update annotations for restore
	...
	if err := r.Create(ctx, pod); err != nil {
		r.Log.Error(err, "Failed to create pod during restore", "podName", pod.Name)
		return nil, err
	}
	r.Log.Info("Pod created successfully", "podName", pod.Name, "creationTime", time.Since(podCreationStart).Milliseconds())
	r.Log.Info("Total restore process duration", "pod", pod.Name, "duration", time.Since(startRestore).Milliseconds())
